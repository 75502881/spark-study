...
public class BestSaleInfo {
	public static void main(String[] args) {
		...
		SparkSession spark = SparkSession.builder().appName("BestSaleInfo")
				.config("spark.sql.warehouse.dir", warehouseLocation).master("spark://Hadoop-NameNode:7077").enableHiveSupport().getOrCreate();
		...
		JavaRDD<String> rowsRdd = spark.sparkContext()
				.textFile("hdfs://192.168.31.231:9000/user/root/input/sales.txt", 1).toJavaRDD();
		...
		System.out.println("------------------------filterRDD start------------------------------");
		...
		System.out.println("------------------------filterRDD end------------------------------");
		...
		Dataset<Row> dailyTop3KeywordDF = spark.sql("" + "SELECT date,keyword,uv " + "FROM (" + "SELECT " + "date,"
